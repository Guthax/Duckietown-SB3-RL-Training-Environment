...
class ResizeWrapper(gym.ObservationWrapper):
    def __init__(self, env=None, shape=(80, 160, 3)):
    ...
    def observation(self, observation):
        ...
        return resize(observation, self.shape, preserve_range=True)
...
class MultiInputWrapper(gym.ObservationWrapper):
    def __init__(self, vae, env):
        ...
        self.observation_space = spaces.Dict({
            ...
            "vae_latent": spaces.Box(low=-4, high=4, shape=(64,), dtype=np.float32)
            #"vehicle_measures": gym.spaces.Box(low=np.array(low), high=np.array(high), shape=(2,), dtype=np.float32)
        })
        self.vae = vae

    def observation(self, obs):
        observation = self.encode_state(obs)
        return observation
    ...
    def preprocess_frame(self, frame):
        ...
        frame = preprocess(frame).unsqueeze(0).float()
        return frame
